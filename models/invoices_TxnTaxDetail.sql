  ...
  create or replace table `honu-dev-1`.test_customer_b2cf8d57_842e_4971_86af_716a17d07cd3_quickbooks.`invoices_TxnTaxDetail`
  partition by timestamp_trunc(_airbyte_emitted_at, day)
  cluster by _airbyte_emitted_at
  OPTIONS()
  as (
    
with __dbt__cte__invoices_TxnTaxDetail_ab1 as (
...
),  __dbt__cte__invoices_TxnTaxDetail_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__invoices_TxnTaxDetail_ab1
select
    _airbyte_invoices_hashid,
    cast(TotalTax as 
    float64
    ...
    _airbyte_emitted_at,
    CURRENT_TIMESTAMP() as _airbyte_normalized_at
from __dbt__cte__invoices_TxnTaxDetail_ab1
-- TxnTaxDetail at invoices/TxnTaxDetail
where 1 = 1

),  __dbt__cte__invoices_TxnTaxDetail_ab3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__invoices_TxnTaxDetail_ab2
select
    ...
    tmp.*
from __dbt__cte__invoices_TxnTaxDetail_ab2 tmp
-- TxnTaxDetail at invoices/TxnTaxDetail
where 1 = 1

)-- Final base SQL model
-- depends_on: __dbt__cte__invoices_TxnTaxDetail_ab3
select
    _airbyte_invoices_hashid,
    TotalTax,
    TxnTaxCodeRef,
...
from __dbt__cte__invoices_TxnTaxDetail_ab3
-- TxnTaxDetail at invoices/TxnTaxDetail from `honu-dev-1`.test_customer_b2cf8d57_842e_4971_86af_716a17d07cd3_quickbooks.`invoices_scd`
where 1 = 1
