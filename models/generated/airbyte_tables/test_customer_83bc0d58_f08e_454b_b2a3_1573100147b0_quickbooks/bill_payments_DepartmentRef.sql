    ...
    post_hook = ["
                    {%
                        set scd_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='bill_payments_DepartmentRef_scd'
                        )
                    %}
                    {%
...
-- depends_on: {{ ref('bill_payments_DepartmentRef_ab3') }}
select
...
-- DepartmentRef at bill_payments/DepartmentRef from {{ ref('bill_payments') }}
where 1 = 1
