  ...
  create or replace table `honu-dev-1`.test_customer_b2cf8d57_842e_4971_86af_716a17d07cd3_quickbooks.`refund_receipts_Line`
  partition by timestamp_trunc(_airbyte_emitted_at, day)
  cluster by _airbyte_emitted_at
  OPTIONS()
  as (
    
with __dbt__cte__refund_receipts_Line_ab1 as (
...
),  __dbt__cte__refund_receipts_Line_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__refund_receipts_Line_ab1
select
    _airbyte_refund_receipts_hashid,
    cast(LineNum as 
    int64
    ...
    _airbyte_emitted_at,
    CURRENT_TIMESTAMP() as _airbyte_normalized_at
from __dbt__cte__refund_receipts_Line_ab1
-- Line at refund_receipts/Line
where 1 = 1

),  __dbt__cte__refund_receipts_Line_ab3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__refund_receipts_Line_ab2
select
    ...
    tmp.*
from __dbt__cte__refund_receipts_Line_ab2 tmp
-- Line at refund_receipts/Line
where 1 = 1

)-- Final base SQL model
-- depends_on: __dbt__cte__refund_receipts_Line_ab3
select
    _airbyte_refund_receipts_hashid,
    LineNum,
    Description,
...
from __dbt__cte__refund_receipts_Line_ab3
-- Line at refund_receipts/Line from `honu-dev-1`.test_customer_b2cf8d57_842e_4971_86af_716a17d07cd3_quickbooks.`refund_receipts_scd`
where 1 = 1
